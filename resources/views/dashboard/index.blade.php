...
@section('title', 'Dashboard - Aditya\'s Web')
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
    ...
    <div class="glass-premium p-6 rounded-3xl flex items-center gap-5 transition-transform duration-500 hover:-translate-y-2 hover:shadow-xl hover:shadow-purple-500/20 animate-fade-up">
        ...
        <div>
            ...
            <h3 class="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-900 to-purple-800 mt-1">{{ $totalArtikel ?? 5 }}</h3>
        </div>
    </div>
    ...
    <div class="glass-premium p-6 rounded-3xl flex items-center gap-5 transition-transform duration-500 hover:-translate-y-2 hover:shadow-xl hover:shadow-blue-500/20 animate-fade-up animation-delay-200">
        ...
        <div>
            ...
            <h3 class="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-900 to-purple-800 mt-1">{{ $totalPenulis ?? 2 }}</h3>
        </div>
    </div>
    ...
    <div class="glass-premium p-6 rounded-3xl flex items-center gap-5 transition-transform duration-500 hover:-translate-y-2 hover:shadow-xl hover:shadow-pink-500/20 animate-fade-up animation-delay-400">
        ...
        <div>
            ...
            <h3 class="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-900 to-purple-800 mt-1">{{ $totalKategori ?? 4 }}</h3>
        </div>
    </div>
</div>
...
<div class="glass-premium rounded-[2.5rem] p-8 md:p-10 animate-fade-up animation-delay-400">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        ...
        <a href="{{ route('artikel.index') }}" class="text-purple-700 font-extrabold text-sm hover:text-indigo-900 transition-colors flex items-center gap-1">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="text-indigo-900/80 text-sm font-medium">
                ...
                <tr class="border-b border-white/30 hover:bg-white/40 transition-colors duration-300">
                    <td class="py-4 px-4 font-extrabold text-indigo-950">{{ $artikel->judul }}</td>
                    <td class="py-4 px-4">
                        <span class="px-4 py-1.5 rounded-full bg-gradient-to-r from-purple-100 to-indigo-100 text-purple-700 text-xs font-extrabold shadow-sm">
                            {{ $artikel->kategori->nama_kategori ?? 'Folklore' }}
                        </span>
                    </td>
                    <td class="py-4 px-4">
                        <span class="px-4 py-1.5 rounded-full shadow-sm {{ ($artikel->status ?? 'Published') == 'Published' ? 'bg-emerald-100/80 text-emerald-700' : 'bg-amber-100/80 text-amber-700' }} text-xs font-extrabold flex items-center gap-2 w-max">
                            <span class="w-2 h-2 rounded-full {{ ($artikel->status ?? 'Published') == 'Published' ? 'bg-emerald-500' : 'bg-amber-500' }}"></span>
                            {{ $artikel->status ?? 'Published' }}
                        </span>
                    </td>
                    <td class="py-4 px-4 text-right">
                        <a href="{{ route('artikel.edit', $artikel->id ?? 1) }}" class="text-purple-600 hover:text-purple-900 font-extrabold transition-colors px-3 py-1.5 rounded-lg hover:bg-purple-100/50">Edit</a>
                    </td>
                </tr>
                @empty
                <tr>
